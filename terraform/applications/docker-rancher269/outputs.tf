output "information_next_steps" {
  value = <<-EOF
    ...
    It is up at: ${harvester_virtualmachine.dr269-vm.network_interface[0].ip_address}
    With the respective ports open of: 80 & 443
    Also!
    The VM's SSH User is: ubuntu
    With the password of: ${var.DR269_VM_PW}
    You'll still want to get the bootstrapd password for Rancher v2.6.9 via something like:
    ```
    ssh-keygen -f ~/.ssh/known_hosts -R "${harvester_virtualmachine.dr269-vm.network_interface[0].ip_address}"
    ssh -oStrictHostKeyChecking=no ubuntu@${harvester_virtualmachine.dr269-vm.network_interface[0].ip_address} 
    sudo su
